...
- **上游 Desktop 启动/退出 spike**：在 macOS arm64 补齐 Harness workspace 依赖后观察到 Electron 和 `dsh web` authenticated ready URL；通过 macOS app quit 验证 graceful shutdown、exit 0 和无残留 Host。启动期间有 bounded HTTP 503 inventory warning，Session/workspace 行为尚未验证。
