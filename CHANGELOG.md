...
- **Agent review 入口**：ChangesPanel 按 `workspace/changes` 的 turn changed paths 加载当前 Git diff，并可通过 `agent.review` 读取 Host `changes.summary`/`changes.diff` 前后 diff；新增 Main-owned bounded `pnpm test`/`npm test` runner、取消和退出清理，fixture 已验证。watcher 冲突、test-result-to-turn 关联和 per-hunk review 尚未完成。
